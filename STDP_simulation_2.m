...
% apply the stimulus
    if i_step>=1 && i_step<=2000
        I_mf(p_index) =0.0;
    elseif i_step>=2001 && i_step<=8000
        I_mf(p_index) =random('poisson',3,size(p_index));
    else
        I_mf(p_index) =0.0;
    end
   ...
   % cv_w(i_step) = std(tmp(index),0,'all')/mean(tmp(index),'all');
    ...
    g_ampa_mf(mf_fired) = g_ampa_mf(mf_fired) + f_ext*(1-g_ampa_mf(mf_fired));
...
ylabel('Firing rate of a group of neurons (Hz)', 'FontSize', 12, 'FontName', 'Times'); 
hold off;
...
lgd = legend([h1, h2, h3], {'target neuron population', 'excitatory neuron population', 'inhibitory neuron population'}, 'FontSize', 10.5, 'FontName', 'Times');
set(lgd, 'Position', [0.7, 0.8, 0.1, 0.1]); % [left, bottom, width, height]
set(lgd, 'Box', 'off');
print('square_wave_300dpi.png', '-dpng', '-r300');
